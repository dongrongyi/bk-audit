...
class RuleAuditSQLBuilder:
    ...
    def _build_outer_sql(self, sql_config: SqlConfig, rules: List[RuleFilterConfig]) -> str:
        ...
        # 3. 最外层 select 列表
        #    3.1 event_data => 系统策略可 map_config 映射直传（来源表列），否则 UDF 拼装全部 select 字段
        #    3.2 strategy_id => 系统策略可 map_config 映射（来源表列），否则取策略自身ID
        #    3.3 strategy_rule_id => 系统策略可 map_config 映射（来源表列），否则取命中规则标识
        #    3.4 其他字段 => 来自 field_mapping
        #    槽位字段的映射覆盖仅对系统策略开放（系统初始化策略需透传手工事件的原值）；
        #    普通策略无条件走默认输出，误配映射会产生重复列、Flow 启动失败（快速失败，不静默错绑归属）
        allow_slot_override = self.strategy.source == StrategySource.SYSTEM.value
        select_fields = []
        if not allow_slot_override or EventMappingFields.EVENT_DATA.field_name not in field_mapping:
            select_fields.append(make_json_expr(json_obj_args).as_(EventMappingFields.EVENT_DATA.field_name))
        if not allow_slot_override or EventMappingFields.STRATEGY_ID.field_name not in field_mapping:
            select_fields.append(ValueWrapper(self.strategy.strategy_id, EventMappingFields.STRATEGY_ID.field_name))
        if not allow_slot_override or EventMappingFields.STRATEGY_RULE_ID.field_name not in field_mapping:
