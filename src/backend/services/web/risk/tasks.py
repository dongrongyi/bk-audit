...
def manual_add_event(data: list):
    ...
    for event in data:
        ...
        # 获取风险元信息（优先级：规则 > 策略）；规则必须属于当前策略，避免快照取到其他策略的规则
        ...
        manual_events.append(
            ManualEvent(
                ...
                # 保存 strategy_rule（写 attname：FK 描述符不接受整型，直接赋 strategy_rule=<int> 会抛 ValueError）
                strategy_rule_id=rule.rule_id if rule else None,
                # 保存风险元信息快照
