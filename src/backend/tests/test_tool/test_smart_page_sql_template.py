...
class SmartPageAPITestCase(TestCase):
    ...
    def test_execute_smart_page_tool(self):
        with (
            patch("services.web.tool.executor.tool.api.bk_base.debug_query_sync.bulk_request") as mock_bulk,
            patch("services.web.tool.resources.ExecuteTool._get_user_allowed_scopes", return_value=([], [])),
        ):
...
class GetToolInputVariableCandidatesTest(TestCase):
    ...
    def test_candidate_request_level_full_pipeline(self, mock_execute):
        ...
        resp = GetToolInputVariableCandidates().request(
            {"uid": self.tool.uid, "raw_name": "game_ids"}
        )
